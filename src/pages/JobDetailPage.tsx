...
function parseUnifiedDiff(raw: string): SplitDiffRow[] {
  if (!raw?.trim()) return [];
  ...
  return raw.split(/\r?\n/).map((line) => {
    if (
      ...
      return { type: "meta", oldText: line, newText: line };
    }
    if (line.startsWith("+")) {
      return { type: "add", oldText: "", newText: line.slice(1) };
    }
    if (line.startsWith("-")) {
      return { type: "del", oldText: line.slice(1), newText: "" };
    }
    if (line.startsWith(" ")) {
      const content = line.slice(1);
      return { type: "context", oldText: content, newText: content };
    }
    return { type: "context", oldText: line, newText: line };
  });
}
...
export default function JobDetailPage() {
  ...
  const diffRows = parseUnifiedDiff(job.diffSummary ?? "");
            ...
            <>
              ...
              <div className="diff-viewer card">
                <div className="diff-viewer-header">
                  <div>
                    <h3 className="diff-title">In-app Diff Viewer</h3>
                    <div className="diff-viewer-sub">
                      Syntax-highlighted split diff on Job Detail Page — no need
                      to open GitHub.
                    </div>
                  </div>
                </div>
                <div className="diff-viewer-grid">
                  <div className="diff-column">
                    <div className="diff-column-title">Original</div>
                    {diffRows.map((row, index) => (
                      <div
                        key={`old-${index}`}
                        className={`diff-line ${row.type === "del" ? "line-removed" : row.type === "meta" ? "line-meta" : "line-context"}`}
                      >
                        {row.oldText}
                      </div>
                    ))}
                  ...
                  <div className="diff-column">
                    <div className="diff-column-title">Changed</div>
                    {diffRows.map((row, index) => (
                      <div
                        key={`new-${index}`}
                        className={`diff-line ${row.type === "add" ? "line-added" : row.type === "meta" ? "line-meta" : "line-context"}`}
                      >
                        {row.newText}
                      </div>
                    ))}
