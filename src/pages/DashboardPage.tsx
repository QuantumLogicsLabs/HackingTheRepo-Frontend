  ...
  const load = useCallback(async (): Promise<void> => {
    try {
      ...
      nextJobs.forEach((job) => {
        const previousStatus = previousStatuses.current[job._id];

        if (previousStatus && previousStatus !== job.status) {
          if (job.status === "completed") {
            addNotification({
              title: "Job complete",
              message: `${repoName(job.repoUrl)} is finished and ready for review.`,
              type: "success",
            });
          }
          ...
          if (job.status === "failed") {
            addNotification({
              title: "Job failed",
              message: `${repoName(job.repoUrl)} failed. Check the job for errors.`,
              type: "error",
            });
          }
        }
        ...
        if (
          ...
          addNotification({
            title: "Needs review",
            message: `${repoName(job.repoUrl)} completed but needs your review.`,
            type: "info",
          });
        }

        previousStatuses.current[job._id] = job.status;
      });
    ...
    <div className="page fade-in">
      <div className="page-header">
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          {isAdmin && (
            <span className="admin-pill">ADMIN: viewing all jobs</span>
          )}
          <Link
            to="/jobs/new"
